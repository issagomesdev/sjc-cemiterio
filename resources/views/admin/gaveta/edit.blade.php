...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.gaveta.update", [$gavetum->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cemiterio') ? 'is-invalid' : '' }}" name="cemiterio_id" id="cemiterio_id">
                    @foreach($cemiterios as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cemiterio_id') ? old('cemiterio_id') : $gavetum->cemiterio->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ossuario') ? 'is-invalid' : '' }}" name="ossuario_id" id="ossuario_id">
                    @foreach($ossuarios as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ossuario_id') ? old('ossuario_id') : $gavetum->ossuario->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('indentificacao') ? 'is-invalid' : '' }}" type="text" name="indentificacao" id="indentificacao" value="{{ old('indentificacao', $gavetum->indentificacao) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('descricao') ? 'is-invalid' : '' }}" type="text" name="descricao" id="descricao"value="{{ old('descricao', $gavetum->descricao) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('gaveta_vazia') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="gaveta_vazia" id="gaveta_vazia" value="Sim" {{ $gavetum->gaveta_vazia || old('gaveta_vazia', 'Não') === 'Sim' ? 'checked' : '' }}>
